...
from gems_styles import (
    ...
    add_formatted_runs, add_body_formatted, add_bullet_formatted
)
...
def export_homework(md_path, output_path):
    ...
    r_inst = p_inst.add_run("Hướng dẫn: Học sinh làm bài ra giấy hoặc điền trực tiếp vào phiếu bài tập.")
    r_inst.italic = True
    r_inst.font.name = GEMSFonts.BODY
    r_inst.font.size = GEMSFonts.SIZE_INFO
    r_inst.font.color.rgb = GEMSColors.GRAY
    ...
    while i < len(lines):
        ...
        if current_part == 'I' or (current_part is None and mcq_count < 18):
            ...
            if current_part == 'I':
                p_body = add_body_formatted(doc, stripped, is_blockquote=is_blockquote, size=GEMSFonts.SIZE_SMALL)
        ...
        if stripped.startswith('#'):
            ...
            p_h = add_heading(doc, "", level=level)
            add_formatted_runs(p_h, stripped.lstrip('#').strip(), is_bold_all=True)
        elif stripped.startswith('- ') or stripped.startswith('* '):
            p = add_bullet_formatted(doc, stripped, is_blockquote=is_blockquote)
            if embedded_img_path:
                add_image(doc, embedded_img_path, "")
        else:
            p_body = add_body_formatted(doc, stripped, is_blockquote=is_blockquote, size=GEMSFonts.SIZE_SMALL)

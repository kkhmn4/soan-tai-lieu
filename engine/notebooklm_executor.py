...
def generate_notebooklm_prompt(output_dir, lesson_slug, lesson_title):
    """
    Generate the focus prompt file containing custom pedagogical and design rules for GEMS v8.0.
    """
    ...
    # Prompt template for NotebookLM (strictly Vietnamese formatting for headings/labels)
    prompt_content = f"""# NOTEBOOKLM PROMPT TRUNG TÂM - {lesson_title}
> **HƯỚNG DẪN:** Hãy sao chép toàn bộ nội dung dưới đây và dán trực tiếp vào khung chat của Google NotebookLM sau khi đã tải các file tài liệu nguồn (`{lesson_slug}_dac_ta_gems.md`, `{lesson_slug}_ke_hoach_bai_day.md`, `{lesson_slug}_phieu_hoc_tap.md`, `{lesson_slug}_huong_dan_slide.md`) làm sources.
...
1. **Nguyên tắc thiết kế:**
   ...
   - Nội dung trên Infographic phải bám sát câu hỏi, sơ đồ điền khuyết và bài tập của Phiếu học tập (`{lesson_slug}_phieu_hoc_tap.md`).
...
"""
    ...
    console.print(f"  [green]✓ Đã sinh file prompt NotebookLM tại: {prompt_path}[/green]")
    return prompt_path
...
def execute_notebooklm_pipeline(output_dir, lesson_slug, lesson_title, notebook_id=None):
    ...
    prompt_path = generate_notebooklm_prompt(output_dir, lesson_slug, lesson_title)
    
    if not notebook_id:
        console.print("[yellow]ℹ Không có --notebook-id được cung cấp. Bỏ qua bước tự động gọi nlm CLI.[/yellow]")
        console.print("[yellow]  Giáo viên có thể sao chép file prompt ở trên để chạy thủ công trên giao diện web.[/yellow]")
        return False
    ...
    slide_cmd = f'nlm create slides {notebook_id} --focus "@{prompt_path}" --language vi --confirm'
    ...
    info_cmd = f'nlm create infographic {notebook_id} --focus "@{prompt_path}" --language vi --confirm'
